...
class Scheduler(object):
    ...
    def hello(self, test):
        print("%s scheduler process_id[%s] : %d")
        test()
    ...
    def scheduler(self, type, job_id, test):
        print("%s Scheduler Start" % type)
        if type == 'interval':
            self.sched.add_job(self.hello, test, seconds=10, id=job_id, args=(test,))
        elif type == 'cron':
...
if __name__ == '__main__':
    ...
    #scheduler.scheduler('cron', "1", test)

    # interval 스케쥴러를 실행시키며, job_id는 "2" 입니다.
    scheduler.scheduler('interval', "2",test)
